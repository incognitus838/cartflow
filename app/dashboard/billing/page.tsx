...
import { formatStoreLimit, formatTeamLimit, PLANS } from "@/lib/plans";
...
export default async function BillingPage() {
  ...
  return (
    <>
      <PageHeader
        title="Subscription"
        description="Pro adds team seats. Enterprise unlocks multiple stores."
      />
      ...
      <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        ...
        {business.plan !== "PRO" && business.plan !== "ENTERPRISE" ? (
          <p className="mt-4 text-xs text-slate-500">
            Upgrade to <span className="font-medium text-slate-700">Pro</span> for 5 team members, or{" "}
            <span className="font-medium text-slate-700">Enterprise</span> for 2+ stores.
          </p>
        ) : null}
      </section>
